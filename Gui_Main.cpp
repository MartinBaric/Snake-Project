...
int main()
{
    RenderWindow window(VideoMode(800, 800), "Snake Game",Style::Close);

    bool Loose = false;
    Vertex Snake_pos;
    Snake_pos.position = {400,400};
    ...
    std::cout << Random_Rec[0];
    std::cout << Random_Rec[1];
    ...
    {
        Event event;
        if (Snake_pos.position.x == 0 or Snake_pos.position.x == window.getSize().x or Snake_pos.position.y == 0 or Snake_pos.position.y == window.getSize().y)
           Loose = true;
        while (window.pollEvent(event))
        {
            ...
            if (event.key.code == Keyboard::Down)
                Snake_pos.position.y = Snake_pos.position.y + 10; 
                // Key up pressed
            if (event.key.code == Keyboard::Up)
                Snake_pos.position.y = Snake_pos.position.y - 10; 
                // Key left pressed
            if (event.key.code == Keyboard::Left)
                Snake_pos.position.x = Snake_pos.position.x - 10;
                // Key right pressed
